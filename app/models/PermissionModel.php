  ...
  public function updatePermissions($role_id, $permissions)
  {
    ...
    if (!empty($permissions)) {
      foreach ($permissions as $menu_id => $perms) {
        ...
        $can_read = (isset($perms['create']) || isset($perms['update']) || isset($perms['delete']) || isset($perms['convert']) || isset($perms['read'])) ? 1 : 0;

        $this->db->query('INSERT INTO role_permissions (role_id, menu_id, can_read, can_create, can_update, can_delete, can_convert) VALUES (:role_id, :menu_id, :can_read, :can_create, :can_update, :can_delete, :can_convert)');
        $this->db->bind(':role_id', $role_id);
        ...
        $this->db->bind(':can_convert', isset($perms['convert']) ? 1 : 0);
        $this->db->execute();
      }
    }
    return true;
